#!/bin/bash
# fix-promedio-sede.sh - CREA SERVICIO DE PROMEDIOS POR INSTANCIA

echo "====================================================="
echo "📊 CREANDO SERVICIO DE PROMEDIOS POR INSTANCIA"
echo "====================================================="
...
if [ -f "$INDEX_FILE" ]; then
    ...
    # Agregar uso del router
    if ! grep -q "app.use('/api/instance/average'" "$INDEX_FILE"; then
        sed -i '/app.use(.api.metric-history./a app.use(\x27/api/instance/average\x27, instanceAverageRoutes);' "$INDEX_FILE"
    fi
...
CRON_JOB="*/5 * * * * cd ${BACKEND_DIR}/.. && node scripts/calculate-averages.js >> /var/log/kuma-averages.log 2>&1"
...
echo "   • Almacena promedios cada 5 minutos por instancia"
echo "   • avgResponseTime, avgStatus, monitorCount"
echo ""
echo "2️⃣ NUEVAS FUNCIONES EN SQLITE:"
echo "   • calculateInstanceAverage() - Calcula promedio de una instancia"
echo "   • calculateAllInstanceAverages() - Calcula promedios de todas"
echo "   • getInstanceAverage() - Obtiene serie histórica de promedios"
echo ""
echo "3️⃣ NUEVO ENDPOINT EN API:"
echo "   • GET /api/instance/average/:instanceName - Serie de promedios"
echo "   • GET /api/instance/average/:instanceName/latest - Último promedio"
echo "   • POST /api/instance/average/calculate - Forzar cálculo"
echo ""
echo "4️⃣ DATOS DE PRUEBA GENERADOS:"
echo "   • 10 instancias"
echo "   • Últimas 24 horas de datos"
echo "   • Promedios calculados automáticamente"
echo ""
echo "5️⃣ MANTENIMIENTO AUTOMÁTICO:"
echo "   • Cron job cada 5 minutos"
echo "   • Script manual: node scripts/calculate-averages.js"
echo ""
echo "🔄 PRUEBA AHORA EN EL FRONTEND:"
echo ""
echo "   1. Abre http://10.10.31.31:5173"
echo "   2. Activa el modo oscuro (botón 🌙)"
echo "   3. Entra a UNA SEDE (Caracas, Guanare, etc.)"
echo "   4. ✅ DEBE mostrar la gráfica de promedio INMEDIATAMENTE"
echo "   5. ✅ El selector de tiempo DEBE funcionar"
echo ""
echo "📌 VERIFICACIÓN MANUAL:"
echo ""
echo "   # Verificar promedios de Caracas:"
echo "   curl 'http://10.10.31.31:8080/api/instance/average/Caracas?hours=24' | jq '.'"
echo ""
echo "   # Verificar estadísticas:"
echo "   curl 'http://10.10.31.31:8080/api/metric-history/stats' | jq '.'"
echo ""
echo "   # Forzar cálculo manual:"
echo "   curl -X POST 'http://10.10.31.31:8080/api/instance/average/calculate'"
echo ""
echo "====================================================="

# Preguntar si quiere probar el endpoint
read -p "¿Probar endpoint de promedios ahora? (s/N): " TEST_ENDPOINT
if [[ "$TEST_ENDPOINT" =~ ^[Ss]$ ]]; then
    echo ""
    echo "📊 Probando endpoint de promedios..."
